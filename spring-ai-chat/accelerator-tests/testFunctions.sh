...
{
  if ! grep -Fq "$2" $1; then
    echo "${RED}$1 DOES NOT contain \"$2\"${NC}"
  fi
}
...
{
  if grep -Fq "$2" $1; then
    echo "${RED}$1 SHOULD NOT contain \"$2\"${NC}"
  fi
}
...
assertPomHasProjectCoordinates()
{
  ...
  if [ ${#DEPRESULT} -eq "0" ]; then
    echo "${RED}$1 does not have project coordinates \"$2:$3\"${NC}"
  fi
}
...
assertPomHasDependency()
{
  ...
  DEPRESULT=`xpath -e $DEPTEST -q $1 2>/dev/null`
  if [ ${#DEPRESULT} -eq "0" ]; then
    echo "${RED}$1 does not contain dependency \"$2:$3:$4\"${NC}"
  fi
}
